...
const datasets = ([[]] as Vec3[][]).concat(await Promise.all(availableDatasets.map((name) => {
    return fetch(`datasets/${name}.csv`)
        .then(res => res.text())
        .then((text) => {
            const obj = Papa.parse<Vec3Like>(text, { dynamicTyping: true, skipEmptyLines: true });
            return obj.data.map(value => new Vec3(value));
        });
})));
init();

function generatePointCloud(): Vec3[] {
...
function rebuild(group: THREE.Object3D) {
    group.clear();
...
function ExportToSVG(rendererSVG: SVGRenderer, filename: string) {
