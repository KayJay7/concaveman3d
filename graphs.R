...
timeModel <- lm(sample$times ~ sample$sizes + I(sample$sizes^2), sample)
timeModelCubic <- lm(sample$times ~ sample$sizes + I(sample$sizes^2)+ I(sample$sizes^3), sample)
...
print(summary(timeModel))
print(summary(timeModelCubic))

par(mfrow = c(1, 1))
plot(sample)
lines(sample$sizes, predict(timeModel), col = "red")
lines(sample$sizes, predict(timeModelCubic), col = "#0bc828")
...
# print(sample)
# sample.x115 <- sample[-115]

# model <- lm(y ~ x + x2, data = sample)
